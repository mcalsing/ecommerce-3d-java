...
class PrinterService:
    ...
    def process_order(self, order: OrderKafka):
        ...
        try:
            ...
            logger.info(f"Preparando impressão com cores:")
            logger.info(f"  - LampShade: {order.lampShadeColor}")
            logger.info(f"  - LampBase: {order.lampBaseColor}")
            
            # 4. Enviar para impressão (exemplo básico)
            # NOTA: A API bambulabs_api pode ter métodos específicos para envio de jobs
            # Ajuste conforme a documentação da sua versão
    ...
    def _notify_spring_boot(self, order_id: int, status: str):
        """Notifica Spring Boot sobre status da impressão (opcional)"""
        try:
            # Endpoint do seu Spring Boot para atualizar status
            url = f"http://localhost:8080/orders/{order_id}/status"
            response = requests.put(url, json={"status": status})
            logger.info(f"Spring Boot notificado: {status}")
        except Exception as e:
            logger.warning(f"Não foi possível notificar Spring Boot: {e}")
